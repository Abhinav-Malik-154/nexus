...
class AnomalyDetector:
    ...
    def _rule_based_detection(self, data: Dict) -> Tuple[bool, List[str]]:
        ...
        elif 'tvl_change_1h' in data:
            # Handle pre-calculated percentage change
            tvl_change_1h = data['tvl_change_1h'] / 100.0  # Convert percent to decimal
            if tvl_change_1h < -self.thresholds['tvl_drop_1h']:
                reasons.append(f"TVL dropped {abs(tvl_change_1h)*100:.1f}% in 1 hour")
        ...
        elif 'tvl_change_1d' in data:
            # Handle pre-calculated percentage change 
            tvl_change_1d = data['tvl_change_1d'] / 100.0  # Convert percent to decimal
            if tvl_change_1d < -self.thresholds['tvl_drop_24h']:
                reasons.append(f"TVL dropped {abs(tvl_change_1d)*100:.1f}% in 24 hours")
